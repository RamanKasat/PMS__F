...
/**
	...
	public PensionDetail calculatePensionAmount(PensionerDetail pensionDetail) {
		...
		if (pensionDetail.getPensionType().equalsIgnoreCase("self") && (bankType.equalsIgnoreCase("public"))) {
			
				pensionAmount = (pensionDetail.getSalary() * 0.8 + pensionDetail.getAllowance());
				bankCharges = 500;
			}
		else if (pensionDetail.getPensionType().equalsIgnoreCase("self") && (bankType.equalsIgnoreCase("private"))) {
			
			pensionAmount = (pensionDetail.getSalary() * 0.8 + pensionDetail.getAllowance());
			bankCharges = 550;
		}
		
		if (pensionDetail.getPensionType().equalsIgnoreCase("family") && (bankType.equalsIgnoreCase("public"))) {
			
			pensionAmount = (pensionDetail.getSalary() * 0.5 + pensionDetail.getAllowance());
			bankCharges = 500;
		}
	else if (pensionDetail.getPensionType().equalsIgnoreCase("family") && (bankType.equalsIgnoreCase("private"))) {
